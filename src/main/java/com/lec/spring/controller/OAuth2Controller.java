...
public class OAuth2Controller {
    ...
    private KakaoProfile kakaoUserInfo(String accessToken) {
        ...
        //사용자 정보(json) > 자바(카카오프로파일 객체 만든거)로 받아내기
        ObjectMapper mapper = new ObjectMapper();
    ...
    private User registerKakaoUser(KakaoProfile profile) {
        ...
        if (user == null) { //미가입자일 때만 회원가입
            ...
            if(cnt > 0) {
                ...
                user = userService.findByUsername(username);  // 다시 읽어와야 한다. id, regDate 등 우리가 저장하지 않은 정보
            } else {
                System.out.println("[Kakao 인증 회원 가입 실패]");
            }

        } else {
            System.out.println("이미 가입해써 빢빢머가리야");
        }
    ...
    private void loginKakaoUser(User kakaoUser) {
        ...
        System.out.println("깎까오 린증 로긘 처리 완료");
    }
}
