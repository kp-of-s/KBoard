...
//                .httpBasic(Customizer.withDefaults())
                ...
                .formLogin(form -> form
                        ...
                        .loginProcessingUrl("/user/login")  // "/user/login" url 로 POST request 가 들어오면 시큐리티가 낚아채서 처리, 대신 로그인을 진행해준다(인증).
                                                            // 이와 같이 하면 Controller 에서 /user/login (POST) 를 굳이 만들지 않아도 된다! (검증코드는 우리가 만듦)
                                                            // 위 요청이 오면 자동으로 UserDetailsService 타입 빈객체의 loadUserByUsername() 가 실행되어 인증여부 확인진행 <- 이를 제공해주어야 한다.
                        .defaultSuccessUrl("/") //로그인 끝날 시 이동할 url ("/")는 원래 가려던 페이지. 직접 url 접근 시 /home으로
...
//                        .failureHandler(AuthenticationFailureHandler)
                ...
                .logout(httpSecurityLogoutConfigurer -> httpSecurityLogoutConfigurer
                        .logoutUrl("/user/logout")  //로그아웃 수행할 URL
                        .invalidateHttpSession(false) //즉시 세션 청소할건가? 기본값 true. 만약 세션에서 활용할 정보가 있다면 f로
                        // 이따가 CustomLogoutSuccessHandler 에서 꺼낼 정보가 있기 때문에
                        // false 로 세팅한다. 이거 쓰면 핸들러에서 수동삭제 필요
                        // .deleteCookies("JSESSIONID")   // 쿠키 제거
//                        .logoutSuccessUrl("/user/login?logout") //로그아웃 성공 후 리다렉 url. 간단한 구현에 쓰기 좋음
                        // 로그아웃 성공후 수행할 코드
                 ...
                 ********************************************/
                // ※ 아래 설정이 없이 (권한없는)'user2' 로 /board/write 접근하면 403 에러 발생
