...
import com.lec.spring.config.oauth.provider.Facebook2UserInfo;
...
public class PrincipalOauth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        OAuth2User oAuth2User = super.loadUser(userRequest);    //로그인한 사용자 정보 가져오기
       ...
       """.formatted(userRequest.getClientRegistration()  // Clie   ntRegistration
        ...
        OAuth2UserInfo oAuth2UserInfo = switch (provider.toLowerCase()){
            ...
            case "facebook" -> new Facebook2UserInfo(oAuth2User.getAttributes());
       ...
       """.formatted(username, name, email, password, provider, providerId));
        ...
        if (user == null) {     //새로 가입하는 경우 진행
            ...
            System.out.println("가입한 빢빢머가리");
        }

        PrincipalDetails principalDetails = new PrincipalDetails(user, oAuth2User.getAttributes());
        principalDetails.setAuthorityRepository(authorityRepository);   //권한 체크
